...
router.post('/', isLoggedIn, upload.none(), async (req, res, next) => {   // POST /post
  try {
    ...
    if (req.body.image) {
      if (Array.isArray(req.body.image)) {    // 이미지를 여러 개 올리면 ['제로초.png', '부기초.png']
        const images = await Promise.all(req.body.image.map(image => Image.create({ src: image })));
        await post.addImages(images);
      }
      else {    // 이미지를 하나만 올려민 image: 제로초.png
...
router.post('/:postId/retweet', isLoggedIn, async (req, res, next) => {   // Post /post/1/retweet
  try {
    ...
    const retweetWithPrePost = await Post.findOne({
      where: { id: retweet.id },
      include: [{
        model: Post,
        as: 'Retweet',
    ...
    return res.status(201).json(retweetWithPrePost);
  }
  catch (error) {
    console.error(error);
    next(error);
